...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/">
        ...
        <html>
            ...
            <!-- navigation section -->
            <hr/>
            <p class="nav row justify-content-md-center">
        ...
        <xsl:variable name="src">
            <xsl:choose>
                <xsl:when test="contains(., 'atropos')">
                    <xsl:text>http://www.borrowers.uga.edu/</xsl:text><xsl:value-of select="substring-after(., 'atropos.english.uga.edu')"/>
                </xsl:when>
                <xsl:when test="contains(., 'klotho')">
                    <xsl:text>http://www.borrowers.uga.edu/</xsl:text><xsl:value-of select="substring-after(., 'klotho.english.uga.edu')"/>
                </xsl:when>
                <xsl:when test="contains(., 'bandl')">
                    <xsl:text>http://www.borrowers.uga.edu/</xsl:text><xsl:value-of select="substring-after(., 'bandl.english.uga.edu')"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="."/>
                </xsl:otherwise>
        ...
        <xsl:variable name="uri">
            <xsl:choose>
                <xsl:when test="contains(@uri, 'atropos')">
                    <xsl:text>http://www.borrowers.uga.edu/</xsl:text><xsl:value-of select="substring-after(@uri, 'atropos.english.uga.edu')"/>
                </xsl:when>
                <xsl:when test="contains(@uri, 'bandl')">
                    <xsl:text>http://www.borrowers.uga.edu/</xsl:text><xsl:value-of select="substring-after(@uri, 'bandl.english.uga.edu')"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="@uri"/>
                </xsl:otherwise>
